 ...
 ** ****************************************** **/
	...
	if($_REQUEST['action'] = 'email_send') {
		...
		$m->AltBody 	= 'Para ver el mensaje, por favor use un email compatible con HTML!';

		$m->AddAddress($config['send_to'], 'Dadelec.cl');
		$m->AddReplyTo($array['email'], $array['name']);
		$m->SetFrom($config['smtp_user'], 'Dadelec.cl');
		$m->MsgHTML("
			Se ha enviado la siguiente cotización a través de Dadelec.cl: <br><br>

			<b>Fecha:</b> {$date} <br> 
			<b>Nombre:</b> {$array['name']}<br>
			<b>Email:</b> {$array['email']}<br>
			<b>Asunto:</b> {$array['subject']}<br><br>
			<b>Mensaje:</b> {$array['message']}<br>
